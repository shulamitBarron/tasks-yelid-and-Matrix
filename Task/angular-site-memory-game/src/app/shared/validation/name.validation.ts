...
export function createValidatorArrName(cntName: string, min: number, max: number): Array<ValidatorFn> {
    return [
...
export function createValidatorArrAge(cntName: string, min: number, max: number): Array<ValidatorFn> {
    return [
        f => !f.value ? { "val": `${cntName} is required` } : null,
